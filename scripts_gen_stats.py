...
user = gh_json(['api', 'users/NhanSoHocCode'])
repos = gh_json(['repo', 'list', 'NhanSoHocCode', '--limit', '100', '--json', 'name,isPrivate,stargazerCount,forkCount,primaryLanguage,pushedAt'])
...
try:
    contrib = gh_json(['api','graphql','-f','login=NhanSoHocCode','-f',f'query={query}'])
...
overview = f'''<svg width="495" height="220" viewBox="0 0 495 220" fill="none" xmlns="http://www.w3.org/2000/svg">{common}<rect width="495" height="220" rx="16" fill="url(#bg)" stroke="#164e63"/><text class="title" x="24" y="38">GitHub Stats</text><text class="label" x="28" y="78">Public repositories</text><text class="value" x="230" y="80">{user.get('public_repos', len(public))}</text><text class="label" x="28" y="112">Followers</text><text class="value" x="230" y="114">{user.get('followers',0)}</text><text class="label" x="28" y="146">Total stars</text><text class="value" x="230" y="148">{stars}</text><text class="label" x="28" y="180">Total forks</text><text class="value" x="230" y="182">{forks}</text><circle class="pulse" cx="430" cy="52" r="20" fill="#34d399" opacity=".35"/><circle class="pulse" cx="430" cy="52" r="7" fill="#34d399"/></svg>'''
